...
function ensureLedgerDay(state: Pick<GameState, "day" | "cash" | "ledger">): DailyLedgerEntry[] {
...
function startLedgerDay(state: GameState, day: number): GameState {
...
function cleanTestDestroyChance(item: InventoryItem, revealed: ItemCondition) {
...
function grantApprovalChance(state: GameState, definition: GrantDefinition): number {
  const progress = labProgress(state);
  const stats = infrastructureStats(state.ownedInfrastructure, state.labStations);
  const impactScore =
...
function grantPayout(definition: GrantDefinition): number {
  return roll(definition.payoutRange[0], definition.payoutRange[1]);
}

function approvedGrantMessage(definition: GrantDefinition): string {
  const messages = [
...
function rejectedGrantMessage(): string {
  const messages = [
...
function processGrantDay(state: GameState): { state: GameState; messages: string[] } {
  let next = state;
  const messages: string[] = [];
